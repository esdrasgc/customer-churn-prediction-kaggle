...
def plot_target_distribution(df: pl.DataFrame, target: str = 'Churn'):
    ...
    axes[0].pie(target_counts.values, labels=['Não Churn', 'Churn'], autopct='%1.1f%%', startangle=90)
...
def plot_categorical_analysis(df: pl.DataFrame, categorical_vars: List[str], target: str = 'Churn'):
...
def plot_numerical_distributions(df: pl.DataFrame, numeric_vars: List[str], target: str = 'Churn'):
...
def plot_boxplots_by_target(df: pl.DataFrame, numeric_vars: List[str], target: str = 'Churn'):
...
def plot_pairplot_sample(df: pl.DataFrame, vars_to_plot: List[str], target: str = 'Churn', sample_size: int = 1000):
    ...
    g = sns.pairplot(df_sample[vars_subset], hue=target, diag_kind='hist', plot_kws={'alpha': 0.6})
